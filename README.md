...
渡鸦当前固定出现在正式封存区 Boss 前的安全屋。靠近渡鸦按 `F` 打开军械面板，再按 `F` 进入武器对比确认，确认后购买一把玩家尚未拥有的武器。当前售卖武器也会显示在渡鸦柜台上，库存现已包含检疫霰弹枪。当本商人没有可售新武器时，终端会切换为基础补给交易，循环提供破障炸药和生物识别钥。随机临时黑市留到后续版本。
