...
func play_fire_windup(target_position: Vector2, duration: float) -> void:
	if _visual_node != null:
		...
		_visual_tween.tween_property(_visual_node, "scale", _visual_base_scale * 1.12, duration)
		_visual_tween.parallel().tween_property(_visual_node, "modulate", Color(0.62, 0.92, 1.0, 1.0), duration)
	...
	warning.z_index = -1
	warning.width = 3.0
	warning.default_color = Color(0.36, 0.78, 1.0, 0.72)
	...
	warning_tween.tween_property(warning, "modulate", Color(0.7, 0.96, 1.0, 0.12), maxf(0.08, duration))
